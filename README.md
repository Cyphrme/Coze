...
is ignored for `tmb` generation. Like `typ` in key may be used to specify custom
application fields, e.g. "first_seen".  
...
For example, if needing to sign image, the digest of the image may be included
...
Pros:
 ...
 - From a base conversion perspective, Hex is correct: 
   ...
   - Lower case hex not a truncation of any (current) industry alphabet. 
   ...
   - Both hex alphabets are extensions of binary, quaternary (base4), octal
     (base8), and base10 (decimal), although duodecimal is typically written with majuscule. 
...
In Hex, 1/16 of hashes will start with 0 and 1/256 will start with a 00, which
is a whole "empty" byte.  When encoding decimal preceding 0's are typically
...
Also, excluding short digests, Coze explicitly recommends against including
